...
- **Helper scripts:** the scripts under `scripts/helpers/` (tool helpers, `*.sh`) and `scripts/hooks/` (hook scripts, `*.py`) are installed into `~/.local/bin` under stable `keru-*` names and referenced by name or via `~/.local/bin`, so the rules stay portable across machines. The installer also adds `~/.local/bin` to the shell profile if missing. Two kinds:
  - **Hook helpers** (run by hooks in `config/hooks.json`, see [permissions.md](permissions.md)): `keru-safe-read` (the Bash command gate: fast static read-only approval plus a model fallback for the unknowns), `keru-gate-deliverable` (`PreToolUse` Write/Edit gate that denies a non-compliant deliverable file before it is written), `keru-block-webfetch` and `keru-block-inline-interp` (the two block hooks), `keru-require-skill` (explicit-skill `Stop` gate), `keru-check-output` (`Stop` gate enforcing a deliverable skill's Output opening, plus no em dashes, English, and well-formed markdown), `keru-judge-output` (`Stop` gate that sends a finished deliverable to a headless Claude judge for the tone/verification rules a regex cannot check), `keru-check-drift` (`SessionStart` hook, injected by the installer with the machine's repo path, that warns when the local clone is behind origin or the repo changed since the last install; see [permissions.md](permissions.md)), `keru-session-brief` (`SessionStart` prints where the previous session left off, `UserPromptSubmit` warns only when a fat session is about to re-cache after an idle gap; model-free, and callable by hand as `keru-session-brief brief`).
  - **Tool helpers** (called by skills/commands to do read-only or mechanical work, with one write exception): `keru-jira-dev` (Jira dev-panel), `keru-jira-set-components` (the one write helper: sets an existing issue's Components field by numeric id, so the token stays off argv; held at `ask`, used by `/keru-create-ticket`), `keru-bot-triage` (dependency/security bot PR + alert triage), `keru-branch-cleanup` (stale local-branch audit/clean), `keru-repo-update` (switch each repo to its default branch and fast-forward to origin: audit/update), `keru-cache-clean` (purge regenerable dev caches, old tool versions, and unused Docker data: audit/clean), `keru-usage` (token spend and estimated cost per month/session/day, plus Stop-gate blocks and the rework they forced, read from the local Claude Code transcripts: month/months/sessions/blocks), `keru-context-snapshot` (walk a ticket's chain with no model into a reusable `~/.claude/keru-context/<KEY>.md`, and verify later that it is still current: write/check/path), `keru-artifacts-prune` (collect the artifacts kept under `~/.claude`: context snapshots after 7 days, gated deliverables after 30, measured by mtime, which for a snapshot means last verified since `check` touches it: audit/prune), `keru-claude-update` (update the Claude Code CLI through the dp `ai` plugin and refresh which models `/model` offers, reading the catalogue from the Bedrock inference profiles rather than a hardcoded list: audit/cli/models).
- **Artifacts on disk:** the two things that must outlive a session live under the Claude config dir, created by the installer: `~/.claude/keru-context/<KEY>.md` (ticket-chain snapshots) and `~/.claude/keru-deliverables/` (the gated deliverable drafts). Not `/tmp`, which was the original home: on this machine nothing user-owned there survived a reboot (a snapshot written one afternoon was gone the next morning, and the daily `tmp_cleaner` only collects what is untouched for 3 days, so it was not the cause), which is shorter than the work the files are meant to outlive. Nothing collects the new location automatically, so `keru-artifacts-prune` does it by hand.
