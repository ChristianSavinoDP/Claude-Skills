...
- DataDog reads (`pup`): `error-tracking issues search/get`, `traces search/aggregate`, `logs search/aggregate/list/query`, `events search/list`, `metrics query/search/list`, `monitors search/get/list`, `slos list/get/status`, `auth status`. Every `pup` write (`cases create/jira`, `metrics submit`, `metrics metadata update`, `traces metrics create/update/delete`, `logs archives/metrics delete`) is in ask, below
...
- DataDog writes (`pup`): `cases create/jira`, `metrics submit`, `metrics metadata update`, `traces metrics create/update/delete`, `logs archives/metrics delete` (remote mutations, out of scope for the read-only audit)
...
**Fast path (instant, deterministic).** The script parses the command with shell-aware tokenizing (pipelines, loops, conditionals, safe redirections like `2>/dev/null`, and command substitution whose contents are themselves read-only) and auto-approves it the moment every segment is provably read-only or local-reversible: `grep`, `find`, `sed`, `awk`, `cat`, `ls`, `git log/diff/status/fetch` (including the `git -C <clone> fetch` form), `go fmt/build/test/list/get/tool` (bare), `pip list/show`, `python -m py_compile`, `mise ls/registry`, `gh`/`jira` read subcommands (`gh api` only as a GET), `pup` read subcommands (`error-tracking/traces/logs/events/metrics/monitors/slos` search/aggregate/query/get/status, `auth status`), `base64`, ... with no state-changing flags (`sed -i`/`perl -i`, `find -exec/-delete`; `-i` is dangerous only for sed/perl, not `grep -i`), no file redirection, and no arbitrary interpreter. It also auto-approves a command that runs **this repo's own tooling**, its test harness, installer, or a hook/helper script (`python3 repo-health/test-hooks.py`, `./scripts/install.sh`, `bash scripts/uninstall.sh`), recognized by the executed file resolving to a path inside the repo (the installer bakes the machine-specific repo path into the installed hook, since it cannot be committed); this is scoped to the executed file and judged per segment, so a repo script chained before a remote mutation still defers that segment. The read-only majority of commands hit this path and never touch a model.
...
Streamed chat text cannot be validated before you see it, so a skill deliverable is written to a file first and shared from there. A `PreToolUse` hook on `Write`/`Edit` enforces that mechanically: when the filename is `keru-deliverable-<skill>.md` (or `keru-deliverable-<skill>-<id>.md`, where the optional `<id>` is a Jira key or PR number that keeps concurrent or sequential deliverables of the same skill from overwriting each other), it validates the content against that skill's Output contract (the same checkers as `keru-check-output`: the required opening, no em dashes, English, and well-formed markdown) before the file is written. If it does not comply, the hook returns `deny`, so the file is never created and Claude is told why and retries. The `<skill>` segment in the filename selects which contract applies (matched as the longest known skill name, so a hyphenated skill is not split on the `<id>` hyphen). The hook also enforces *where* the draft lands: it must be `~/.claude/keru-deliverables/`, and a write anywhere else (notably `/tmp`, the old convention, which does not survive a reboot) is denied with the absolute path to use, so the location does not depend on the model having loaded the right skill file. A second scope also applies: any Markdown (`.md`) file written into a repo (not a `/tmp` or `var/folders` scratch path, and not a `node_modules`/`.git`/`vendor`/`.venv`/`site-packages` tree) is itself a deliverable and must clear the shared form rules (no em dashes, English, well-formed markdown) before it is written. Every other Write/Edit, meaning non-Markdown files and `.md` files under a temp or vendored tree, is untouched. This is the one place enforcement is mechanical rather than reactive: unlike a Stop hook (which runs after the message is shown), a denied write means the bad deliverable never reaches disk.
...
- **`keru-check-output`:** enforces a deliverable skill's Output contract, the machine-checkable part. Each deliverable skill defines a concrete opening (pr-review opens with the verdict line, writing-tickets/pr-description/bot-triage/datadog-audit/addressing-pr-comments with a bold header, investigation with a markdown heading); once the opening is confirmed, three Playbook-wide body rules are checked: no em dashes (including the fenced block pasted into a PR), English only, and well-formed markdown (code fences closed, lists blank-line-separated). It runs only when the message clearly IS the deliverable (a clarifying question or trailing content after a correct opening is left alone), and it detects the deliverable even when the skill was never explicitly loaded, via its structural form.
- **`keru-judge-output`:** the second layer, for the rules a regex cannot check (tone, claims asserted without verification, a remembered-but-wrong shape, content the deliverable's reader would not act on). When a turn produces a deliverable for `pr-review`, `writing-tickets`, `pr-description`, `addressing-pr-comments`, `bot-triage`, or `datadog-audit`, it sends the delivered text plus the skill's rules to a headless Claude judge (`dp ai claude`) that reads it as a fresh reviewer and blocks once if it clearly violates the skill. It runs only after the regex gate says the form is right (so it never judges chat or doubles up), and it is excluded for `investigation` because that skill already runs its own adversarial-review subagent. Cost: one short model call per actual deliverable, none on ordinary turns. This is the mechanical version of "have a second pair of eyes check it," for the judgment-level rules Claude kept failing to self-apply.

## The session brief (`keru-session-brief`)

Two hooks, one helper, both model-free: everything it prints is extracted mechanically from the local transcripts, so it costs nothing to produce and can never invent a fact.

- **`SessionStart` (mode `brief`):** prints where the previous session in this project left off (context size at the end, files edited, ticket keys seen and whether a `keru-context-snapshot` exists for them, the last few asks). It fires only when `source` is `startup` or `clear`, because `resume` still has the history, `compact` has just produced a summary, and `fork` inherits its parent, so printing there is noise. Silent when the previous session ended more than a day ago (a different task) or had no real activity.
- **`UserPromptSubmit` (mode `nudge`):** silent on almost every prompt. It prints one line only when the session is both fat (context at or above 150K tokens) and idle (no activity for at least the 5-minute prompt-cache TTL), i.e. when this turn is about to re-cache a large prefix, and it quotes what that re-cache costs. Because it runs on the critical path of every prompt, it checks the cheap signal first (the transcript mtime) and only parses the tail of the file if that passes. Thresholds are env-overridable: `KERU_BRIEF_CTX_TOKENS`, `KERU_BRIEF_IDLE_SECONDS`, `KERU_BRIEF_MAX_AGE_HOURS`.

The point is not to save tokens by injecting context, which would add to every turn what it claims to save. A `/clear` is the only thing that shrinks a fat context, and the reason not to clear is losing your place; this makes clearing cheap, and clearing is what saves. What it deliberately does not do is summarize decisions or rationale: that needs a model, and it says so in its own output, so nothing there is mistaken for a verified account of what was decided.
...
- **Behind origin:** HEAD is behind the remote default branch. So this reflects the remote in the current session (not whenever you last fetched by hand), the check refreshes the refs first with a throttled, timeout-bounded `git fetch --prune`: at most once every few hours (gated on the mtime of `FETCH_HEAD`) and abandoned if the remote is slow, so most sessions pay nothing and a hung remote never stalls startup. Only done when you are actually on the default branch, so feature-branch sessions pay no network cost and are not nagged. Fail-open: a skipped or failed fetch just falls back to the existing local refs.
