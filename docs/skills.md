...
| `keru-datadog-audit` | audit DataDog errors for a set of services | read-only via `pup`; groups by recurrence and judges attribution, routes to `keru-debugging` (cause in doubt) or `keru-writing-tickets` (recurring + ours), both separate steps |
