...
| `keru-cache-clean` | purge regenerable dev caches, old tool versions, and unused Docker data | typed-only; confirms against an audit first. Everything removed regenerates/re-downloads (Go module cache, npm, pip, brew old versions, `docker system prune`). Browser and OS/app caches left alone |
| `keru-artifacts-prune` | collect old context snapshots and gated deliverables from `~/.claude` | typed-only; confirms against an audit first. A snapshot rebuilds on demand (7-day default), a deliverable does not (30-day default). Age is mtime, which for a snapshot means last verified (`check` touches it). Local but irreversible for deliverables |
| `keru-claude-update` | update Claude Code itself and refresh which Claude models `/model` offers | typed-only; confirms against an audit first. Installs a new CLI build through `dp ai`, then writes the model env slots (newest per family from the Bedrock profiles, `[1m]` spelling optional) and the pinned model into `settings.json`, backed up. Local; only the model keys are touched |
