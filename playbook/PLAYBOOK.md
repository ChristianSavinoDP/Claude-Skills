...
- **Maximum detail: self-review before delivering, as a gate.** Do not deliver code until you have re-read the full diff as a strict reviewer would (Copilot included) and it would pass. This is a step you must actually perform, not an intention. Cover, at minimum: nil safety, error handling, unused imports, correct types and field names, missing returns; AND the bugs that compile fine but only fail at runtime, which are the ones reviewers keep catching: struct serialization and tags (`omitempty`, JSON/YAML tags marshaling empty or wrong values), config strings parsed at runtime, reflection, interface assertions, edge cases and empty inputs. If a reviewer could reasonably flag it, you should have caught and fixed it first. If unsure whether something is correct, verify it (read the type, run the check), do not assume.
- **Stay within scope.** Do not add features, behavior, or changes beyond what the ticket asks.
- **Jira and GitHub: always the CLI, never WebFetch.** `jira` (jira-cli) and `gh` are installed and authenticated on this machine. A Jira/GitHub URL is just an id: extract it and run the CLI (`jira issue view <KEY>`, `gh pr view <n> --repo <owner>/<repo>`, `gh api ...`). Never WebFetch a Jira/GitHub URL and never look for an MCP server. If the CLI is missing, stop and say so; do not guess.
- **Use the right tool, not raw shell.** Each action goes through the harness tool built for it, not a shell command disguising it. Write or overwrite a file's contents with Write/Edit, not `cat > file`, `tee`, or `> file`, so there is a visible diff, a checkpoint for rewind, and a trail. Read files with the Read tool, not `cat`/`sed` when you just need the contents. Parse with a tool's own flags, not `python3 -c`/`node -e`/`ruby -e`. Moving, copying, or renaming files (`mv`, `cp`) is fine as a local operation; the rule is about authoring content, not relocating files.
...
- **Tests:** when you add non-trivial new logic (branches, edge cases, parsing, anything whose failure would only surface at runtime), add unit tests for it in the same PR, following the repo's existing test pattern, and tell the user you did and why. This is what a reviewer (Copilot included) will ask for anyway. Two limits: only if the repo already tests that kind of code (do not introduce a test framework where there is none), and not for trivial changes (config, rename, wiring). Each test covers a distinct behavior, no redundancy. If the user explicitly says no tests, skip them.
...
1. **Location and code, as a reference (outside any fence):** the `path/to/file.go:42` line and the line(s) of code it refers to, so the user can find it. Not copied to GitHub.
2. **The comment, in its own copy-pasteable fenced block:** only the text that gets pasted into the GitHub comment (the suggestion itself, plus any `suggestion` code block). Wrap it in a four-backtick fence so an inner code block does not break it. This is the only part the user copies.
3. **The why, below, outside the fence:** a short rationale for the user, so they understand the finding before posting it. Not part of the comment.

If everything is good, just the verdict line; do not invent comments to show you reviewed it.
