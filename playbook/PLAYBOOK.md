...
- Do not confuse acting from memory with the saved memory store; they are opposites. The first is the model's own recollection, the mistake above. The second is a checked artifact written down on purpose (the memory files, surfaced through the `MEMORY.md` index each session): when it already holds what a task needs (a cached component id, a saved repo or service list, the projects root), read that file first instead of rederiving the value from an API or re-asking the user, because saving it was the whole point. It is still a cache, so confirm an entry is current before trusting a stale one (an id can be archived or renamed), but consulting it is the fast path to prefer, never a step to skip: skipping it burns tokens rederiving what was already saved, and inventing a value while a memory file holds it is fabrication.
...
1. Write it with the Write tool to `~/.claude/keru-deliverables/keru-deliverable-<skill>-<id>.md`, expanding `~` to your home directory (the Write tool does not expand it, and a literal `~` would create a directory actually named that). `<id>` is the ticket key or PR number the work is about (e.g. `keru-deliverable-pr-review-3254.md`, `keru-deliverable-writing-tickets-DBI-1477.md`). The `<id>` keeps a new deliverable from overwriting an earlier one (a prior run, or another session working in parallel). When the work spans repos or services instead of one ticket (a bot triage, a DataDog audit), the `<id>` is today's date, which is what lets the next run say whether a finding is new; omit it only when there is genuinely no ticket, PR or recurring run. The `<skill>` selects which Output contract the gate enforces. If a turn produces more than one deliverable (e.g. an investigation and then a ticket), each goes to its own file, written and gated separately. Not `/tmp`: that was the old home and it does not survive a reboot, so the gate denies a draft written there and tells you the path to use.
...
- **Concise, natural, no slop.** No filler, no generic intros, no vague conclusions; write like explaining to a colleague. Any deliverable opens with the substance, organized so a reader finds any point by scanning. Before writing, name the reader and what they decide with it; then apply two tests per sentence: delete it, does that reader do anything differently? And would a colleague on this team say this, in this many words? A no means cut the sentence, not shorten it, because compressed slop is still slop. What reads as machine output is the shape more than the length: restating the diff or the data the reader is about to read, explaining the repo's own tooling back to the team that owns it, a bold lead-in on every paragraph, an inventory bullet per changed item, every clause closing an argument nobody contested.
- **The verification trail is the cost of being right, not the product.** What you checked in order to be sure stays behind the output. A rebuttal to a finding only your own review pass raised, a refuted alternative, a factual error you caught in some other ticket: nobody asked for any of it, so none of it belongs in the deliverable. What is true but outside this reader's decision gets routed, not published: a comment on the ticket it concerns, a comment in the code, a word to the person affected. And never record the same thing twice; a decision already stated in the change's own comments is not restated in prose.
- **No em dashes.** Use commas, semicolons, colons, or parentheses.
- **Well-formed markdown.** Every deliverable must render: close every code fence you open, and put a blank line before and after a list. This is render-correctness, not linter style; the repo deliberately uses long lines and opens deliverables with a bold header, so line-length and first-line-heading rules do not apply. A PreToolUse gate enforces it on the deliverable file.
...
- **Fan out to subagents** when the work is *judgment* that gains from an independent, unbiased pass (reviewing a diff along several dimensions, verifying findings, several analysis lenses). Launch them in a single message so they run concurrently, each owning one dimension with no stake in the others. It costs tokens but catches blind spots one pass rationalizes away. A subagent's finding is a claim to verify against the source yourself, never a fact to repeat; you own the synthesis, the agents only return raw findings.
  - **Match the model to the leg.** A fan-out mixes judgment legs with mechanical ones; do not spend the top model on both. Give a leg that is real judgment (hunting runtime bugs, weighing acceptance criteria, attribution, adversarial review) the strong model; route a mechanical leg (running the repo's tests/lint, fetching CI logs, per-service/per-repo read gathering, log triage) to a cheaper, faster one (`model: 'haiku'` or `'sonnet'` on the Agent call). Same parallelism, less cost and latency. When in doubt whether a leg is judgment, keep it on the strong model.
