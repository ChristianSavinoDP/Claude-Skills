...
- **Re-verify the whole contract, not just the flagged lines.** When a comment touches a claim the PR makes about a backing source that can move (a schema, a proto, a type, an API contract), do not patch only the flagged field: open the *current* source (the merged one, if it landed since the diff was written) and re-read the entire message, type, or schema the comment belongs to, re-deriving every claim the change makes about it. This is the `keru-writing-code` "verify what the change rests on, not only the lines it touches" rule applied to this flow: addressing one field while adjacent facts about the same contract stay stale is what forces a second review round, and correcting those adjacent facts is fixing the deliverable, not over-correcting.
