...
This chain is discover-then-read (Playbook "Parallelize the work"): a step that depends on the previous one's output stays serial, but the items *within* a step are independent I/O once discovered, so read them concurrently. Concretely: you cannot batch step 2 until step 1 names the issues, nor step 4 until step 3 identifies the investigation, but once you have the set, issue those reads together. Keep it concurrent I/O, not subagents: the value here is understanding the *why* behind the chain, which does not survive being handed to a blind agent, so you read and synthesize every result yourself.

1. **Read the ticket's raw fields** (`jira issue view <KEY> --raw`) to see parent, epic, and every linked issue. The `--plain` view hides links; the raw JSON does not. This is the discovery step: it names the set the next step reads.
2. **Fetch every linked issue and the parent/epic** (`jira issue view <LINKED-KEY> --plain --comments 10`). These are independent of each other, so issue the reads concurrently and collect the results, do not walk them serially. Read them, do not just list them.
...
- A PR's changed files and diff: read the pushed PR head, which is what a review must read. **Prefer the local clone whenever the repo is cloned; it is faster than paging the whole PR over the API, and most of the user's repos are cloned under the projects root (memory `projects-root`, one level deep).** Going remote per file when a clone exists is the slow default this skill is correcting. Get the head into the clone read-only, without a checkout and without touching the user's current branch:
  1. `git -C <clone> fetch origin pull/<n>/head` (auto-approved: it mutates nothing but the local `FETCH_HEAD` and does NOT switch branches, so the Bash gate's model check clears it; the `git fetch *` allow rule does not, since this form leads with `git -C`).
