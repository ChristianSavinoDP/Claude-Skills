...
This chain is discover-then-read (Playbook "Parallelize the work"), and the two halves are handled differently:

- **Discovery is never delegated.** Step 1 names the set, and it is the Playbook's mandatory first step: you get the ticket yourself before anything fans out. An agent cannot be sent to find a chain whose shape nobody knows yet.
- **Reading the set is delegated, on a cheap model.** Once step 1 names the linked issues, parent, epic, investigation and PRs, those items are independent, so dispatch one subagent per item in a single message and pick the model per item (Playbook "Match the model to the leg"): `model: 'haiku'` for mechanical retrieval (a status, an epic's children, a PR's file list, whether a doc exists at a path), `model: 'sonnet'` where the item needs reading comprehension (an investigation doc, a task breakdown, a comment thread where the scope moved).
- **A reader returns a distillation with pointers, never a dump and never a vibe summary:** what the item is, the facts that bear on *this* ticket, verbatim quotes for the decisive lines (an acceptance criterion, a recorded correction), and pointers (`file:line`, PR number, doc path) for the rest. Keeping the bulk out of the main context is the entire point; an agent that pastes the document back has done nothing for you.
- **Then read the decisive lines yourself, at the source.** A returned quote is a claim to verify (Playbook), and the rationale behind an acceptance criterion is exactly what must not be acted on second-hand. Delegating the fetch is fine; delegating the interpretation is not.

1. **Get the ticket, yourself, and snapshot the chain in the same pass.** Run `keru-context-snapshot check <KEY>`; if it is not `current`, run `keru-context-snapshot write <KEY>`. That walks one hop out with no model at all (root, parent, epic, every link, plus the dev panel's PRs) into `~/.claude/keru-context/<KEY>.md` and returns a small JSON map: the keys it found, the linked PRs, the path. **Then read the root ticket's section in that file in full** (description, acceptance criteria, comments) and its `Chain` list. This is the Playbook's mandatory first step and it is not delegable: you read the ticket, never an agent's summary of it. If a field looks missing, `jira issue view <KEY> --raw` is still the authority (the `--plain` view hides links; the raw JSON does not).
2. **Fan the set out to readers.** With the keys named, dispatch one subagent per linked issue, parent and epic, on the model the item deserves (see the delegation rules above), each returning a distillation with pointers rather than the text. The snapshot already holds their descriptions, so a reader's job is judgment about relevance to this ticket, not retrieval: point it at the file section and let it answer "does this bear on the work, and which lines decide it". Read those lines yourself afterwards.
...
- If I worked from a snapshot, did `check` report `current` in *this* session, and did I read the decisive lines myself rather than trusting a reader's paraphrase?
...
Then tell the user what you read (linked tickets, the investigation and where its conclusions live, PRs, files) before proceeding, and say in that same line whether the chain came from a snapshot `check` reported `current` or from a fresh `write`. One clause, not a section: the snapshot is otherwise invisible to the user, who then cannot tell a reused chain from a re-walked one, or notice that a snapshot they expected to exist does not.
