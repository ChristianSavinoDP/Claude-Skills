...
One condition short-circuits all of this before any of it runs: if Phase 1 finds the PR conflicts with its base branch, stop there and report it directly. A conflicting diff will change once the author rebases or merges, so there is nothing stable to review and the fan-out would only produce findings against code that will not ship.
...
3. **Fetch metadata and fail fast on merge conflicts.** Get metadata with `gh pr view <pr>`, and read its mergeability explicitly: `gh pr view <pr> --json mergeable,mergeStateStatus`. If `mergeable` is `CONFLICTING` (equivalently `mergeStateStatus: DIRTY`), stop the whole review right here: do not read the full diff, do not pull the conversation, do not check out the branch, and launch no Phase 2 agent. A diff that conflicts with its base will change once the author rebases or merges, so any finding lands on code that will not ship and the fan-out is wasted. Report it directly in chat, one line, no gated deliverable file: say the PR conflicts with `<base>` and needs a rebase or merge before it can be reviewed, then stop and wait. `mergeable: UNKNOWN` is not a conflict, it means GitHub is still computing the merge check, so re-query once after a moment before trusting it; only a confirmed `CONFLICTING` aborts. Otherwise continue with the diff: follow gather-context's PR-diff guidance, which leads with the local path: if the branch is already checked out at the PR head, read the diff from the working tree (no fetch, no checkout); otherwise read the pushed head from the local clone; go remote (`gh pr diff <pr>`, measuring a large diff before dumping it) only when the repo is not cloned. Do not re-fetch or re-checkout what is already local at the head (Playbook "read local state before re-fetching remote"). For an investigation PR, also check `docs/investigations/` for the expected format. Read changed files in full when behavior depends on surrounding code.
4. **Read the live PR conversation and review state (read-only).** Pull the existing threads and reviews from GitHub yourself (`gh api repos/<owner>/<repo>/pulls/<pr>/comments` and `.../reviews`), not a recap of them. A second-round review ("la segunda vuelta") turns on what was already commented and whether it was addressed; you cannot judge that from memory or a summary. Note the current review state (who approved or requested changes, and at which head). You will need it in Phase 3. And if a prior review draft already sits at the deliverable path (`~/.claude/keru-deliverables/keru-deliverable-pr-review-<pr>.md`), treat every claim in it as unverified text to re-derive against the source, never as evidence: an inherited draft is another pass's assertions, not your verification. Re-deriving each one is the point; a nit that referenced a rename that never happened will read as "still stands" if you carry it forward blind.
...
Write this to `~/.claude/keru-deliverables/keru-deliverable-pr-review-<pr>.md` first, where `<pr>` is the PR number (the Playbook's gated-deliverable rule, so a new review does not overwrite an earlier one); your chat reply is a link to that file plus at most one line, not its pasted contents. Anything you want to add for the user is that one line, after the link, never mixed into the file.
