...
1. Write the doc to `~/.claude/keru-deliverables/keru-deliverable-<token>-<id>.md`, where `<token>` is `doc` in companion mode or `investigation` in investigation mode, and `<id>` is the ticket key or PR number (e.g. `~/.claude/keru-deliverables/keru-deliverable-doc-DBI-1676.md`). The PreToolUse gate validates it (opens with a heading or a bold title, no em dashes, English, well-formed markdown) and DENIES the write if not; fix and Write again until it passes.
